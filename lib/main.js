...
function get_city() {
    ...
    city_lat = JSON.parse(http.responseText).lat
    city_lng = JSON.parse(http.responseText).long
    console.log("lat: " + JSON.parse(http.responseText).lat)
    console.log("long: " + JSON.parse(http.responseText).long)
}
...
L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { maxZoom: 3}).addTo(map);
...
map.on('draw:created', function(e) {
    ...
    if (type === 'marker') {
        ...
        document.getElementById("dist").innerHTML = 'distance: ' + Math.round(getDistance([city_lat, city_lng], [click_lat, click_lng]) / 1000) + ' kilometers'
        count_of_click += 1
        if (count_of_click % 2 == 1){
            ...
            if (player_1 > player_2){
                score[1] += 1;
                document.getElementById("winner").innerHTML = '2 player get point'
                //if (count_of_click = 2) {
                //    change_rand()
            }
            else {
                score[0] += 1;
                document.getElementById("winner").innerHTML = '1 player get point'
                    //if (count_of_click = 2) {
                    //    change_rand()
                    //}
            }
            document.getElementById("score").innerHTML = '1 player - ' + score[0]
            document.getElementById("sco").innerHTML = '2 player - ' + score[1]
            if (count_of_click = 4) {
                get_city()
            }
        }
